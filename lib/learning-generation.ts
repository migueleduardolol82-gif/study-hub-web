...
export const lessonContentSchema = {
  ...
  properties: { studyNotes: string, summary: string, keyConcepts: { type: "array", items: string, maxItems: 12 }, examples: { type: "array", items: string, maxItems: 8 }, commonErrors: { type: "array", items: string, maxItems: 8 }, references: { type: "array", items: string, maxItems: 12 }, exercises: { type: "array", minItems: 4, maxItems: 10, items: {
    type: "object", additionalProperties: false, required: ["type", "prompt", "options", "answer", "explanation", "optionExplanations", "sourceReference", "essentialCriteria", "concept", "difficulty", "teaching", "example", "teachingHighlights", "memoryTip", "comparisonRows"],
    properties: { type: { type: "string", enum: exerciseKinds }, prompt: string, options: { type: "array", items: string, maxItems: 8 }, answer: string, explanation: string, optionExplanations: { type: "array", items: string, maxItems: 8 }, sourceReference: string, essentialCriteria: { type: "array", minItems: 1, maxItems: 12, items: string }, concept: string, difficulty, teaching: string, example: string, teachingHighlights: { type: "array", items: string, maxItems: 6 }, memoryTip: string, comparisonRows: { type: "array", maxItems: 6, items: { type: "object", additionalProperties: false, required: ["term", "explanation"], properties: { term: string, explanation: string } } } },
  } } },
};
...
export function validateLessonContent(value: unknown): Omit<import("./learning.ts").LearningLesson, "id" | "title" | "description" | "difficulty" | "xp"> {
  ...
  const exercises = value.exercises.map((exercise) => {
    ...
    const teachingHighlights = exercise.teachingHighlights;
    if (teachingHighlights !== undefined && (!Array.isArray(teachingHighlights) || teachingHighlights.length > 6 || teachingHighlights.some(item => typeof item !== "string" || !item.trim() || item.length > 300))) throw new Error("Destaques didáticos inválidos.");
    const comparisonRows = exercise.comparisonRows;
    if (comparisonRows !== undefined && (!Array.isArray(comparisonRows) || comparisonRows.length > 6 || comparisonRows.some(row => !isRecord(row) || typeof row.term !== "string" || !row.term.trim() || row.term.length > 200 || typeof row.explanation !== "string" || !row.explanation.trim() || row.explanation.length > 1200))) throw new Error("Comparação didática inválida.");
    return { id: `exercise-${crypto.randomUUID()}`, type, options, answer, prompt: field(exercise, "prompt"), explanation: field(exercise, "explanation", 4000), optionExplanations, sourceReference: field(exercise, "sourceReference", 500), essentialCriteria: essentialCriteria as string[] | undefined, concept: exercise.concept === undefined ? undefined : field(exercise, "concept", 200), difficulty: exercise.difficulty as ThemeDifficulty | undefined, teaching: exercise.teaching === undefined ? undefined : field(exercise, "teaching", 5000), example: exercise.example === undefined ? undefined : field(exercise, "example", 3000), teachingHighlights: teachingHighlights as string[] | undefined, memoryTip: exercise.memoryTip === undefined ? undefined : field(exercise, "memoryTip", 2000), comparisonRows: comparisonRows as Array<{ term: string; explanation: string }> | undefined };
  });
