...
const Widget = memo(function Widget({
	...
	return (
		<motion.div
			...
			aria-label={item.label ?? `Widget ${SIZE_LABELS[item.size]}`}
	...
	const step = useCallback(
		(force = false) => {
			...
			if (!force && now - lastMove.current < SETTLE_MS) {
				// The next pointer frame or drop completes this move.
				return
			}
			...
			<div
				...
				{editable && (
					<p id={hintId} className="sr-only">
						Arraste para reorganizar. No celular, pressione e segure. No teclado,
						mantenha Alt pressionado e use as setas.
					</p>
				)}
				<div
					ref={grid}
