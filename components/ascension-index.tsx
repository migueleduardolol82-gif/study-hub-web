...
import { RoutineSheet } from "@/components/routine-workspace";
...
export function AscensionIndex({ tracks, onChange, journeys, xp, compact = false }: Props) {
  ...
  return <section className="hli panel">
    ...
    {compact ? null : <>
      ...
      {editing && <RoutineSheet title={editing.id ? "Editar habilidade" : "Nova habilidade"} onClose={()=>setEditing(null)}><form className="hli-form" onSubmit={saveTrack} key={editing.id}>
        {error && <p role="alert">{error}</p>}
        ...
        <details><summary>Vínculos e referências opcionais</summary>
        ...
        <label>Dias por semana<input name="weeklyDays" type="number" min="1" max="7" defaultValue={editing.weeklyDays}/></label>
        </details>
      ...
      <div className="hli-tracks">{tracks.filter(t=>showArchived||!t.archived).map(track=>{
        ...
        return <article key={track.id} className="hli-skill"><span className="eyebrow">{track.domain}{track.archived?" · Arquivada":""}</span><h3>{track.name}</h3><p>{track.context}</p>
          ...
          <details><summary>Desempenho, atividades e histórico</summary>
