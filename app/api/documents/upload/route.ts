...
import { MAX_STUDY_FILE_BYTES } from "@/lib/study-documents";
...
export async function POST(request: Request) {
  try {
    ...
    const response = await handleUpload({ request, body, onBeforeGenerateToken: async (pathname) => {
      ...
      return { allowedContentTypes: ["application/pdf", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.openxmlformats-officedocument.presentationml.presentation", "text/plain", "image/jpeg", "image/png"], maximumSizeInBytes: MAX_STUDY_FILE_BYTES, addRandomSuffix: true, tokenPayload: JSON.stringify({ userId }) };
    } });
